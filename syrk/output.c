...
for (int ki = 0; ki < ((K) / (4)); ki++) {
  float *A_panel = malloc(M * 4 * sizeof(*A_panel));
  for (int i0 = 0; i0 < M - 0; i0++) {
    for (int i1 = 0; i1 < 4 * ki + 4 - 4 * ki; i1++) {
      A_panel[(i0) * (4) + (i1) * (1)] = A[(0 + i0) * (K) + (4 * ki + i1) * (1)];
    }
  }
  float *B_panel = malloc(4 * N * sizeof(*B_panel));
  for (int i0 = 0; i0 < 4 * ki + 4 - 4 * ki; i0++) {
    for (int i1 = 0; i1 < N - 0; i1++) {
      B_panel[(i0) * (N) + (i1) * (1)] = B[(4 * ki + i0) * (N) + (0 + i1) * (1)];
    }
  }
  GEPP(ctxt,M,N,(struct exo_win_2f32){ (float*)&A_panel[(0) * (4) + (0) * (1)], { 4, 1 } },(struct exo_win_2f32){ (float*)&B_panel[(0) * (N) + (0) * (1)], { N, 1 } },(struct exo_win_2f32){ (float*)&C[(0) * (N) + (0) * (1)], { N, 1 } });
  free(A_panel);
  free(B_panel);
}
if (K % 4 > 0) {
