...
//     C : [f32][4,4]  @DRAM
// )
void microkernel_4x4_1( c_code_str_Context *ctxt, int_fast32_t K, struct exo_win_2f32 A, struct exo_win_2f32 B, struct exo_win_2f32 C ) {
EXO_ASSUME(K >= 1);
...
for (int ji = 0; ji < ((N) / (16)); ji++) {
  ...
  for (int io = 0; io < M % 4; io++) {
    for (int jo = 0; jo < 16; jo++) {
      for (int k = 0; k < K; k++) {
        C[(io + ((M) / (4)) * 4) * (N) + (16 * ji + jo) * (1)] += A[(io + ((M) / (4)) * 4) * (K) + (k) * (1)] * B[(k) * (N) + (16 * ji + jo) * (1)];
      }
    }
  }
  free(B_strip);
...
for (int ii = 0; ii < ((M) / (4)); ii++) {
  for (int jii = 0; jii < ((N % 16) / (4)); jii++) {
    microkernel_4x4_1(ctxt,K,(struct exo_win_2f32){ (float*)&A[(4 * ii) * (K) + (0) * (1)], { K, 1 } },(struct exo_win_2f32){ (float*)&B[(0) * (N) + (16 * ((N) / (16)) + 4 * jii) * (1)], { N, 1 } },(struct exo_win_2f32){ (float*)&C[(4 * ii) * (N) + (16 * ((N) / (16)) + 4 * jii) * (1)], { N, 1 } });
  }
  for (int io = 0; io < 4; io++) {
    for (int joo = 0; joo < N % 16 % 4; joo++) {
      for (int k = 0; k < K; k++) {
        C[(4 * ii + io) * (N) + (joo + ((N % 16) / (4)) * 4 + ((N) / (16)) * 16) * (1)] += A[(4 * ii + io) * (K) + (k) * (1)] * B[(k) * (N) + (joo + ((N % 16) / (4)) * 4 + ((N) / (16)) * 16) * (1)];
      }
    }
  }
}
