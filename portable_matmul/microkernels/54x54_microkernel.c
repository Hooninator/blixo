...
// microkernel_4x2x52_3(
//     A : [f32][4,52]  @DRAM,
//     B : [f32][52,2]  @DRAM,
//     C : [f32][4,2]  @DRAM
// )
void microkernel_4x2x52_3( c_code_str_Context *ctxt, struct exo_win_2f32 A, struct exo_win_2f32 B, struct exo_win_2f32 C ) {
EXO_ASSUME(A.strides[1] == 1);
EXO_ASSUME(B.strides[1] == 1);
EXO_ASSUME(C.strides[1] == 1);
float32x4_t C_reg[52][1];
for (int j = 0; j < 2; j++) {
  for (int k = 0; k < 52; k++) {
...
for (int j = 0; j < 2; j++) {
  float32x4_t A_vec[52][1];
  float32x4_t B_vec[52];
  for (int k = 0; k < 52; k++) {
    for (int io = 0; io < 1; io++) {
...
for (int ii = 0; ii < ((M) / (4)); ii++) {
  ...
  for (int ko = 0; ko < ((K) / (52)); ko++) {
    for (int jiii = 0; jiii < ((N % 16 % 4) / (2)); jiii++) {
      microkernel_4x2x52_3(ctxt,(struct exo_win_2f32){ (float*)&A[(4 * ii) * (K) + (52 * ko) * (1)], { K, 1 } },(struct exo_win_2f32){ (float*)&B[(52 * ko) * (N) + (4 * ((N % 16) / (4)) + 16 * ((N) / (16)) + 2 * jiii) * (1)], { N, 1 } },(struct exo_win_2f32){ (float*)&C[(4 * ii) * (N) + (4 * ((N % 16) / (4)) + 16 * ((N) / (16)) + 2 * jiii) * (1)], { N, 1 } });
    }
    for (int jooo = 0; jooo < N % 16 % 4 % 2; jooo++) {
      for (int io = 0; io < 4; io++) {
        for (int ki = 0; ki < 52; ki++) {
          C[(4 * ii + io) * (N) + (jooo + ((N % 16 % 4) / (2)) * 2 + ((N % 16) / (4)) * 4 + ((N) / (16)) * 16) * (1)] += A[(4 * ii + io) * (K) + (52 * ko + ki) * (1)] * B[(52 * ko + ki) * (N) + (jooo + ((N % 16 % 4) / (2)) * 2 + ((N % 16) / (4)) * 4 + ((N) / (16)) * 16) * (1)];
        }
      }
    }
  }
  for (int jiii = 0; jiii < ((N % 16 % 4) / (2)); jiii++) {
    for (int jooo = 0; jooo < 2; jooo++) {
      for (int io = 0; io < 4; io++) {
        for (int ki = 0; ki < K % 52; ki++) {
          C[(4 * ii + io) * (N) + (2 * jiii + jooo + ((N % 16) / (4)) * 4 + ((N) / (16)) * 16) * (1)] += A[(4 * ii + io) * (K) + (ki + ((K) / (52)) * 52) * (1)] * B[(ki + ((K) / (52)) * 52) * (N) + (2 * jiii + jooo + ((N % 16) / (4)) * 4 + ((N) / (16)) * 16) * (1)];
        }
      }
    }
  }
  for (int jooo = 0; jooo < N % 16 % 4 % 2; jooo++) {
    for (int io = 0; io < 4; io++) {
      for (int ki = 0; ki < K % 52; ki++) {
        C[(4 * ii + io) * (N) + (jooo + ((N % 16 % 4) / (2)) * 2 + ((N % 16) / (4)) * 4 + ((N) / (16)) * 16) * (1)] += A[(4 * ii + io) * (K) + (ki + ((K) / (52)) * 52) * (1)] * B[(ki + ((K) / (52)) * 52) * (N) + (jooo + ((N % 16 % 4) / (2)) * 2 + ((N % 16) / (4)) * 4 + ((N) / (16)) * 16) * (1)];
      }
    }
  }
}
