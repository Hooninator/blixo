...
// microkernel_4x3x24_3(
//     A : [f32][4,24]  @DRAM,
//     B : [f32][24,3]  @DRAM,
//     C : [f32][4,3]  @DRAM
// )
void microkernel_4x3x24_3( c_code_str_Context *ctxt, struct exo_win_2f32 A, struct exo_win_2f32 B, struct exo_win_2f32 C ) {
EXO_ASSUME(A.strides[1] == 1);
EXO_ASSUME(B.strides[1] == 1);
EXO_ASSUME(C.strides[1] == 1);
...
for (int ii = 0; ii < ((M) / (4)); ii++) {
  ...
  for (int ko = 0; ko < ((K) / (24)); ko++) {
    for (int jiii = 0; jiii < ((N % 16 % 8) / (3)); jiii++) {
      microkernel_4x3x24_3(ctxt,(struct exo_win_2f32){ (float*)&A[(4 * ii) * (K) + (24 * ko) * (1)], { K, 1 } },(struct exo_win_2f32){ (float*)&B[(24 * ko) * (N) + (8 * ((N % 16) / (8)) + 16 * ((N) / (16)) + 3 * jiii) * (1)], { N, 1 } },(struct exo_win_2f32){ (float*)&C[(4 * ii) * (N) + (8 * ((N % 16) / (8)) + 16 * ((N) / (16)) + 3 * jiii) * (1)], { N, 1 } });
    }
    for (int jooo = 0; jooo < N % 16 % 8 % 3; jooo++) {
      for (int io = 0; io < 4; io++) {
        for (int ki = 0; ki < 24; ki++) {
          C[(4 * ii + io) * (N) + (jooo + ((N % 16 % 8) / (3)) * 3 + ((N % 16) / (8)) * 8 + ((N) / (16)) * 16) * (1)] += A[(4 * ii + io) * (K) + (24 * ko + ki) * (1)] * B[(24 * ko + ki) * (N) + (jooo + ((N % 16 % 8) / (3)) * 3 + ((N % 16) / (8)) * 8 + ((N) / (16)) * 16) * (1)];
        }
      }
    }
  }
    ...
    for (int jooo = 0; jooo < 3; jooo++) {
      for (int io = 0; io < 4; io++) {
        for (int ki = 0; ki < K % 24; ki++) {
          C[(4 * ii + io) * (N) + (3 * jiii + jooo + ((N % 16) / (8)) * 8 + ((N) / (16)) * 16) * (1)] += A[(4 * ii + io) * (K) + (ki + ((K) / (24)) * 24) * (1)] * B[(ki + ((K) / (24)) * 24) * (N) + (3 * jiii + jooo + ((N % 16) / (8)) * 8 + ((N) / (16)) * 16) * (1)];
        }
      }
    }
  }
  for (int jooo = 0; jooo < N % 16 % 8 % 3; jooo++) {
    for (int io = 0; io < 4; io++) {
      for (int ki = 0; ki < K % 24; ki++) {
        C[(4 * ii + io) * (N) + (jooo + ((N % 16 % 8) / (3)) * 3 + ((N % 16) / (8)) * 8 + ((N) / (16)) * 16) * (1)] += A[(4 * ii + io) * (K) + (ki + ((K) / (24)) * 24) * (1)] * B[(ki + ((K) / (24)) * 24) * (N) + (jooo + ((N % 16 % 8) / (3)) * 3 + ((N % 16) / (8)) * 8 + ((N) / (16)) * 16) * (1)];
      }
    }
  }
}
