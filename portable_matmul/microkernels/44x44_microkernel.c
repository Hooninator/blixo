...
//     C : [f32][4,12]  @DRAM
// )
void microkernel_4x12_1( c_code_str_Context *ctxt, int_fast32_t K, struct exo_win_2f32 A, struct exo_win_2f32 B, struct exo_win_2f32 C ) {
EXO_ASSUME(K >= 1);
...
for (int ii = 0; ii < ((M) / (4)); ii++) {
  for (int jii = 0; jii < ((N % 16) / (12)); jii++) {
    microkernel_4x12_1(ctxt,K,(struct exo_win_2f32){ (float*)&A[(4 * ii) * (K) + (0) * (1)], { K, 1 } },(struct exo_win_2f32){ (float*)&B[(0) * (N) + (16 * ((N) / (16)) + 12 * jii) * (1)], { N, 1 } },(struct exo_win_2f32){ (float*)&C[(4 * ii) * (N) + (16 * ((N) / (16)) + 12 * jii) * (1)], { N, 1 } });
  }
  for (int io = 0; io < 4; io++) {
    for (int joo = 0; joo < N % 16 % 12; joo++) {
      for (int k = 0; k < K; k++) {
        C[(4 * ii + io) * (N) + (joo + ((N % 16) / (12)) * 12 + ((N) / (16)) * 16) * (1)] += A[(4 * ii + io) * (K) + (k) * (1)] * B[(k) * (N) + (joo + ((N % 16) / (12)) * 12 + ((N) / (16)) * 16) * (1)];
      }
    }
  }
}
