...
int main(int argc, char *argv[]) {
  ...
  printf("  openBLAS %s took %5.1lf ms, or %4.1lf GFLOPS. That's %4.1lf percent of peak.\n",
         op.c_str(), ms_per_gemm, (FLOP_C*1.0e-6)/ms_per_gemm, (((FLOP_C*1.0e-6)/ms_per_gemm)/16)*100);
  
  /* BLIS benchmark */
  int N_TIMES_BLIS = 100;
  ...
  /* EXO benchmark */
  int N_TIMES_EXO = 1000000;
  begin = std::chrono::steady_clock::now();
  for(int times = 0; times<N_TIMES_EXO; times++) {
    SGEMM(nullptr, n, n, n, a.data(), b.data(), c.data());
  }
  end = std::chrono::steady_clock::now();
  duration = std::chrono::duration<double>(end - begin).count();
  ms_per_gemm = duration/N_TIMES_EXO*1.0e3;
  double gflops_exo = (FLOP_C*1.0e-6)/ms_per_gemm;
  log_file << n << ", " << gflops_blas << ", " << gflops_blis << ", " << gflops_exo << '\n';
  printf("-----------------------------------------------------------\n");
  printf("  EXO %s took %5.1lf ms, or %4.1lf GFLOPS. That's %4.1lf percent of peak.\n",
         op.c_str(), ms_per_gemm, (FLOP_C*1.0e-6)/ms_per_gemm, (((FLOP_C*1.0e-6)/ms_per_gemm)/16)*100);
