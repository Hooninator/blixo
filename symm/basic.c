...
//     C : [f32][4,16]  @DRAM
// )
void neon_microkernel( c_code_str_Context *ctxt, int_fast32_t K, struct exo_win_2f32 A, struct exo_win_2f32 B, struct exo_win_2f32 C ) {
EXO_ASSUME(K >= 1);
    ...
    for (int jo = 0; jo < ((((N) / (16))) / (4)); jo++) {
      ...
      for (int im = 0; im < 16; im++) {
        for (int jm = 0; jm < 4; jm++) {
          neon_microkernel(ctxt,64,(struct exo_win_2f32){ (float*)&Atile[(4 * im) * (64) + (0) * (1)], { 64, 1 } },(struct exo_win_2f32){ (float*)&Btile[(0) * (64) + (16 * jm) * (1)], { 64, 1 } },(struct exo_win_2f32){ (float*)&C[(64 * io + 4 * im) * (N) + (64 * jo + 16 * jm) * (1)], { N, 1 } });
        }
      }
    }
  }
...
for (int ko = 0; ko < ((K) / (64)); ko++) {
  for (int io = 0; io < ((((M) / (4))) / (16)); io++) {
    for (int jm = 0; jm < ((N) / (16)) % 4; jm++) {
      for (int im = 0; im < 16; im++) {
        neon_microkernel(ctxt,64,(struct exo_win_2f32){ (float*)&A[(64 * io + 4 * im) * (K) + (64 * ko) * (1)], { K, 1 } },(struct exo_win_2f32){ (float*)&B[(64 * ko) * (N) + (16 * (jm + ((((N) / (16))) / (4)) * 4)) * (1)], { N, 1 } },(struct exo_win_2f32){ (float*)&C[(64 * io + 4 * im) * (N) + (16 * (jm + ((((N) / (16))) / (4)) * 4)) * (1)], { N, 1 } });
      }
    }
  }
